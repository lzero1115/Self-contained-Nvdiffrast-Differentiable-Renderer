...
camera_loc = torch.tensor([1.0,1.0,1.0])
target = torch.tensor([0.0,0.0,0.0])
view_mat = look_at(camera_loc,target)
view_mats = []
view_mats.append(view_mat)

# default env_map
env_path = 'textures/overcast_soil_puresky_2k.hdr'
envmap = torch.tensor(imageio.imread(env_path, format='HDR-FI'), device='cuda')
# Add alpha channel
alpha = torch.ones((*envmap.shape[:2],1), device='cuda')
envmap = torch.cat((envmap, alpha), dim=-1)

# default scene setting
default_setting = {"near_clip": 0.100000,
                   "far_clip": 1000.000000,
                   "fov": 39.597755,
                   "res_x": 960,
                   "res_y": 540,
                   "envmap": envmap,
                   "envmap_scale": 1.00000,
                   "view_mats": view_mats
                   }

class NVDRenderer: # environmental light condition renderer
    """
    ...
    def __init__(self, scene_params, shading=True, boost=1.0):
        ...
        self.view_mats = torch.stack(scene_params["view_mats"]).to('cuda') # project world coordinate into local coordinate
        ...
        self.base_color = torch.tensor([1.0, 0.5, 1.0], device='cuda')
    ...
    def render_backgrounds(self, envmap):
        ...
        rays_norm = -(rays.transpose(0,1) / torch.norm(rays, dim=1)).transpose(0,1) # this operation is feasible with 4th component 0
        ...
        envmap_uvs = torch.stack([0.5 - phi / (2 * np.pi), theta / np.pi], dim=-1)
        #self.bgs = dr.texture(envmap[None, ...], envmap_uvs, filter_mode='linear').flip(1)
        self.bgs = dr.texture(envmap[None, ...], envmap_uvs, filter_mode='linear')
    ...
    def render(self, v, n, f):
        ...
        v_ndc = torch.matmul(v_hom, self.mvps.transpose(1,2)) # transpose for mvp projection since shape of v is (n,3)
        rast = dr.rasterize(self.glctx, v_ndc, f, self.res)[0] # mesh in rasterization pipeline (triangle info)
        if self.shading:
            ...
            light = self.base_color * dr.interpolate(vert_light[None, ...], rast, f)[0]
    ...
    def render_normal_texture(self,v,n,f):
        ...
        v_ndc = torch.matmul(v_hom, self.mvps.transpose(1, 2))  # transpose for mvp projection since shape of v is (n,3)
        rast = dr.rasterize(self.glctx, v_ndc, f, self.res)[0]  # mesh in rasterization pipeline (triangle info)
        # rgb_normals = torch.abs(n * 0.5 + 0.5)
        rgb_normals = n * 0.5 + 0.5
        print("rendering normal mapping...")
        interpolated_normals, _ = dr.interpolate(rgb_normals[None, ...], rast, f)
        print("finished!")
...
def main():
    ...
    eye = torch.tensor([1.5, 1.5, 1.0]) # model toward +z axis
    center = torch.tensor([0.0,0.0,0.0])

    new_view_mat = look_at(eye,center)
    #print(new_view_mat)
    # new_view_mat = [
    #                 [0.84901059, -0.11458844, -0.51580084, 0.00578433],
    #                 [-0.02775529, 0.96518096, -0.26010642, 0.07009595],
    #                 [0.52764634, 0.23514931, 0.81626842, -1.91169118],
    #                 [0.0, 0.0, 0.0, 1.0]
    #             ]
    # new_view_mat = torch.tensor(new_view_mat, dtype=torch.float32)


    default_setting["view_mats"][0] = new_view_mat
    # v = scaling_mesh(v,0.5)
    v_gpu = torch.tensor(v, device='cuda', dtype=torch.float32)
    f_gpu = torch.tensor(f, device='cuda', dtype=torch.float32)
    n_gpu = torch.tensor(normals, device='cuda', dtype=torch.float32)

    my_renderer = NVDRenderer(default_setting, shading=True, boost=3)

    # environmental lighting map
    ref_imgs = my_renderer.render(v_gpu,n_gpu,f_gpu) # actually only 1 image is rendered but nvdrenderer can render several imgs at the same time
    img = (ref_imgs[0, ..., :-1].clip(0, 1).pow(1 / 2.2)).cpu().numpy()
    plt.imsave('img.png', img, origin='lower')

    # normal mapping
    # normal_map = my_renderer.render_normal_texture(v_gpu,n_gpu,f_gpu)
    # img = normal_map.cpu().numpy()[0]
